      ...
      <template v-for="(d, index) in dates" :key="d">
        ...
        <button
          ...
          :class="{
            ...
            'bg-gray-700 text-white': dateProps.datesWithTasks.includes(d.date),
          }"
        >
          <span>
            {{ d.date }}
...
const dateProps = defineProps<{
  ...
  selectedDate: number;
  datesWithTasks: Array;
}>();

console.log(dateProps.selectedDate, "ll");

const dateEmits = defineEmits<{ (e: "selected", v: number): void }>();

const dates = ref<Date[]>([]);

onMounted(() => {
  generateDaysForThatMonth();
});
...
watch(
  ...
  (v) => {
    date.value = null;
    generateDaysForThatMonth(v.month, v.year);
  },
  {
    deep: true,
  }
...
function generateDaysForThatMonth(m = dayjs().month(), y = dayjs().year()) {
  dates.value = [];
